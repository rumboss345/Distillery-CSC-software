...
export function Reports() {
  ...
  return (
    <div>
      ...
      <div className="section">
        <h3 className="section-title">Bottling — {periodLabel}</h3>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1rem' }}>
          Tank bottling empties the source tank on save. Variance is bottled volume minus tank draw
          (negative = spirit unbottled / loss; positive = bottled over tank book).
        </p>
        {filteredBottlings.length === 0 ? (
          <div className="empty-state">
            <p>No bottling runs for {periodLabel.toLowerCase()}.</p>
          </div>
        ) : (
          <div className="table-wrap">
            <table>
